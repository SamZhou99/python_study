...
class BrowserFormImgCode:
    def __init__(self) -> None:
        app_id = "337751"
        app_key = "MpIbRajMUVNTKoYydcLQ6pL/jTwAnme4"
        pd_id = "137751"
        pd_key = "UxrPThiIPPF12gE5+BNxvZy5GdPT5vsT"
    ...
    def Verify(self, img_path):
        pred_type = 30600
        file_name = img_path
        return self.API.PredictFromFile(pred_type, file_name)
...
class FateadmApi:
    ...
    def QueryBalc(self):
        ...
        if rsp.ret_code == 0:
            LOG(
                "query succ ret: {} cust_val: {} rsp: {} pred: {}".format(
                    rsp.ret_code, rsp.cust_val, rsp.err_msg, rsp.pred_rsp.value
                )
            )
        else:
            LOG(
                "query failed ret: {} err: {}".format(
    ...
    def QueryTTS(self, pred_type):
        ...
        if rsp.ret_code == 0:
            LOG(
                "query rtt succ ret: {} request_id: {} err: {}".format(
                    rsp.ret_code, rsp.request_id, rsp.err_msg
                )
            )
        else:
            LOG(
                "predict failed ret: {} err: {}".format(
                    rsp.ret_code, rsp.err_msg.encode("utf-8")
                )
            )
        return rsp
    ...
    def Predict(self, pred_type, img_data, head_info=""):
        ...
        if rsp.ret_code == 0:
            LOG(
                "predict succ ret: {} request_id: {} pred: {} err: {}".format(
    ...
    def Justice(self, request_id):
        ...
        if rsp.ret_code == 0:
            LOG(
                "justice succ ret: {} request_id: {} pred: {} err: {}".format(
                    rsp.ret_code, rsp.request_id, rsp.pred_rsp.value, rsp.err_msg
                )
            )
        else:
            LOG(
                "justice failed ret: {} err: {}".format(
                    rsp.ret_code, rsp.err_msg.encode("utf-8")
                )
            )
        return rsp
    ...
    def Charge(self, cardid, cardkey):
        ...
        if rsp.ret_code == 0:
            LOG(
                "charge succ ret: {} request_id: {} pred: {} err: {}".format(
                    rsp.ret_code, rsp.request_id, rsp.pred_rsp.value, rsp.err_msg
                )
            )
        else:
            LOG(
                "charge failed ret: {} err: {}".format(
                    rsp.ret_code, rsp.err_msg.encode("utf-8")
                )
            )
        return rsp
